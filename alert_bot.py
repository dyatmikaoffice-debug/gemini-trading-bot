...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Bollinger Bands (20, 2) for Consolidation Squeeze Filter
...
async def analyze_signal_with_ai(price: float, action: str, ema9: float, ema50: float, ema200: float, macd_hist: float) -> str:
    ...
    STRICT RULES:
    1. VETO if proposed BUY is taking place at the extreme top of parabolic spikes without pullbacks.
    2. VETO if MACD momentum is actively shrinking against the trade direction.
    ...
    while True:
        try:
            ...
            if "values" in data:
                ...
                bb_width_ma = float(latest['bb_width_ma'])
                ...
                if not is_consolidating:
                    ...
                    logging.info("[CONSOLIDATION] Bollinger Band Squeeze detected: Halting entries.")

                # Step 4: State-Aware Cooldown Guard
                conn = get_db_connection()
                cur = conn.cursor()
