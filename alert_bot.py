    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
            if raw_text in ["/help", "/start"]:
                ...
                # Robust stats handler: explicitly request RealDictCursor and
                # safely handle empty/legacy rows so a malformed result cannot
                # crash the Telegram command.
                conn = None
                cur = None
                try:
                    conn = get_db_connection()
                    ...
                    def _scalar(sql, key):
                        ...
                        # Defensive fallback for any legacy tuple cursor.
                        return row[0] if len(row) else 0

                    total_executed = int(_scalar("SELECT COUNT(*) AS total FROM signals WHERE status = 'EXECUTED'", "total"))
                    total_vetoes = int(_scalar("SELECT COUNT(*) AS vetoes FROM signals WHERE status = 'VETOED'", "vetoes"))
                    total_pending = int(_scalar("SELECT COUNT(*) AS pending FROM signals WHERE status = 'EXECUTED' AND outcome = 'PENDING'", "pending"))
                    tp1_wins = int(_scalar("SELECT COUNT(*) AS tp1_wins FROM signals WHERE outcome LIKE 'WIN (TP1%' OR outcome LIKE 'CLOSED%'", "tp1_wins"))
                    tp2_wins = int(_scalar("SELECT COUNT(*) AS tp2_wins FROM signals WHERE outcome LIKE 'WIN (TP2%'", "tp2_wins"))
                    losses = int(_scalar("SELECT COUNT(*) AS losses FROM signals WHERE outcome LIKE 'LOSS%'", "losses"))
                    ...
                    for t in closed_trades:
                        if isinstance(t, dict):
                            trade = {
                                "action": t.get("action"), "entry_price": t.get("entry_p"),
                                "sl_price": t.get("sl_p"), "tp1_price": t.get("tp1_p"),
                                "tp2_price": t.get("tp2_p"), "exit_price": t.get("exit_price"),
                                "outcome": t.get("outcome_val")
                            }
                        else:
                            # Defensive fallback if a tuple row somehow reaches here.
                            vals = list(t)
                            while len(vals) < 7:
                                vals.append(None)
                            trade = {
                                "action": vals[0], "entry_price": vals[1], "sl_price": vals[2],
                                "tp1_price": vals[3], "tp2_price": vals[4],
                                "exit_price": vals[5], "outcome": vals[6]
                            }
                        trade_pips, _ = compute_trade_pips(trade)
                    ...
                    reply = (
                        ...
                        f"• Net Profit (2x0.01 Lot): *${est_dollar:+.2f}*\n\n"
                    ...
                    await send_telegram_alert(client, f"⚠️ Error querying stats: {type(e).__name__}: {e}", target_chat_id=sender_chat_id)
                finally:
                    try:
                        if cur: cur.close()
                    except Exception:
